...
- **Against the shuffled control**: the control — identical model, schema, feature count, and imputation, with history values deranged within position — **tracked baseline closely and never approached the real arm**. It was slightly worse than baseline in the primary joined view (MAE 72.03 vs 68.93), overall (67.72 vs 67.02), and for QB/RB/TE; it was marginally *better* than baseline for WR (59.31 vs 60.88, ~2.6%) — small enough to be shuffle-draw noise plus the structural has-history/imputation effect, and one reason the P4 repeated-seeds check below is required rather than optional. Its correlations stayed near zero everywhere (joined Pearson 0.12, Spearman 0.07). Net: the sanity arm behaved as designed, and the ~29-point real-arm gap is attributable to the **actual history values**, not to model structure or added columns.
- **Per position** (MAE, baseline → real, shuffled in parentheses): QB 99.8 → 65.5 (102.7); RB 75.9 → 49.6 (76.6); TE 48.8 → 30.4 (52.1); WR 60.9 → 36.4 (59.3). Every position improved 34–40% while its shuffled arm stayed within ±3% of baseline; real-arm Pearson is 0.68–0.76 across all four. No position carried the result, and none degraded.
