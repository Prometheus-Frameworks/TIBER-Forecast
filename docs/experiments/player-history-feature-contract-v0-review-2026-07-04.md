...
| `generator_script_version` | **Required, new** | Not present in the PR #124 proposal. Added because `run_id` (below) must derive from it, and a field a `run_id` derives from must itself be a readable part of the contract instance — otherwise no reviewer could recompute or verify the hash. |
| `run_id` | **Required, amended** | Original description said *what* it should tie together but not *how*, and depended on a `generator_script_version` that wasn't itself part of the contract. Amended to a specified composition rule (§3.3): a deterministic hash of the *full* source identity (`source_dataset_refs.artifact_path`, `.artifact_sha256`, `.promotion_review`) plus `contract_version`, `generator_script_version`, and `generated_at` — not `artifact_sha256` alone, consistent with §4's rule that source identity is path + sha256 + promotion review, never sha256 alone. |
...
### 3.3 `run_id` composition rule (amended: full source identity, not sha256 alone)

```text
run_id = deterministic_hash(
  source_dataset_refs.artifact_path,
  source_dataset_refs.artifact_sha256,
  source_dataset_refs.promotion_review,
  contract_version,
...
The original proposal hashed `artifact_sha256` alone, which is inconsistent with §4's non-negotiable
that source identity is path + sha256 + promotion review, never sha256 alone — two artifacts sharing
bytes under a different path or promotion review would otherwise collapse to the same `run_id`. It
also referenced `generator_script_version` without that value being part of the contract instance
itself, leaving a reviewer unable to recompute the hash from the instance alone; `generator_script_version`
is now its own required field (§2) for exactly that reason.
...
```jsonc
{
  ...
  "contract_version": "0.3.0-reviewed",  // bumped from 0.1.0-proposed: provenance_state and
                                          // validation_status became closed enums, run_id now hashes
                                          // the full source identity instead of sha256 alone, and
                                          // generator_script_version was added as its own required
                                          // field (held at pre-1.0 since this is still not
  ...
  "generator_script_version": "version/revision identifier of the generator script that produced this instance",
  "run_id": "deterministic_hash(source_dataset_refs.artifact_path, source_dataset_refs.artifact_sha256, source_dataset_refs.promotion_review, contract_version, generator_script_version, generated_at)",  // see §3.3
...
| A future implementation issue treats `run_id` as "any unique string" | §3.3 specifies the exact composition rule; a `run_id` that doesn't derive from the full source identity (path + sha256 + promotion review) plus `contract_version`, `generator_script_version`, and `generated_at` is non-conforming. |
