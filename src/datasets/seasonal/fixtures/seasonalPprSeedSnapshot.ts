/**
 * Compact, curated per-player season seed for the seasonal PPR scaffold fixture.
 *
 * This is the SEED used to synthesize a TIBER-Data-shaped weekly scaffold
 * artifact (see `tiberDataWeeklyPprScaffold.ts`). Each entry pairs an
 * approximate 2024-season feature snapshot with the known 2025 full-season PPR
 * outcome for a skill-position player.
 *
 * PROVENANCE / GOVERNANCE: these are approximate historical snapshots,
 * hand-assembled to exercise the loader/harness. They are scaffold-only fixture
 * coverage — not a live or governed TIBER-Data pull, and not model-quality
 * validation data. A `null` 2025 actual exercises the fail-closed path.
 *
 * The `SeasonalPlayerObservation` shape is reused as the seed shape because it
 * already carries exactly the fields the scaffold needs (2024 inputs + 2025
 * actual). Skill positions only (QB/RB/WR/TE).
 */
import type { SeasonalPlayerObservation } from '../../../contracts/seasonalPprBacktest.js';

export const seasonalPprSeedSnapshot: SeasonalPlayerObservation[] = [
  // ----- Quarterbacks -----
  ...
  // Outcome unavailable from TIBER-Data: exercises the fail-closed path (no 2025
  // weekly rows are emitted for this player, so the loader marks it unavailable).
